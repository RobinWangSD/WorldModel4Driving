...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.train_ckpt_path is None:
        ...
        base_exp_dir = "/".join(str(cfg.output_dir).split("/")[:-1])
        # Adapt to our wandb logger structure: base_exp_dir/*/*/*/checkpoints/*.ckpt
        search_pattern = base_exp_dir + "/*/*/*/checkpoints/*.ckpt"
    ...
    if use_wandb:
        ...
        run_name = (
            f"navsim-{cfg.get('experiment_name', 'default')}"
            f"-agent{cfg.agent._target_.split('.')[-1]}"
            f"-bs{cfg.dataloader.params.batch_size}"
            f"-seed{cfg.seed}"
        )
        ...
        wandb_logger = CSVLogger(save_dir=cfg.output_dir)
