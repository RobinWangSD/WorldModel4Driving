...
DEFAULT_OUTPUT_PATH = Path("/hugsim-storage/navsim_trainval_camera_or_window_availability.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Count cumulative OR-window 4-camera availability for NAVSIM trainval window tokens."
    )
...
def label_range(end_offset: int) -> str:
    if end_offset == 0:
        return "t"
    if end_offset == 1:
        return "t||t+1"
    return f"t||...||t+{end_offset}"
...
def collect_counts(args: argparse.Namespace) -> tuple[int, List[int], int]:
    ...
    for log_idx, log_pickle_path in enumerate(log_files, start=1):
        ...
        for window in iter_windows(frames, num_frames):
            ...
            frame_availability = [
                has_all_camera_files(
                    window[current_idx + offset],
                    args.cameras,
            ...
            any_available = False
            for end_offset, available in enumerate(frame_availability):
...
def write_csv(args: argparse.Namespace, total_eligible_tokens: int, counts: Sequence[int]) -> None:
    ...
    with open(args.output_path, "w", newline="") as fp:
        ...
        writer.writeheader()
        for end_offset, available_tokens in enumerate(counts):
            ratio = available_tokens / total_eligible_tokens if total_eligible_tokens else 0.0
            writer.writerow(
                {
                    "range_label": label_range(end_offset),
                    "start_offset": 0,
...
def main() -> None:
    ...
    for end_offset, available_tokens in enumerate(counts):
        ratio = available_tokens / total_eligible_tokens if total_eligible_tokens else 0.0
        print(f"{label_range(end_offset)}: {available_tokens} ({ratio:.6f})")
