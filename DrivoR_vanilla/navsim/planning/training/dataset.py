...
class CacheOnlyDataset(torch.utils.data.Dataset):
    ...
    def _load_valid_caches(
        ...
        idx_cach_path = cache_path / "valid_caches.pkl"
        if idx_cach_path.exists():
            print(f"Loading valid cache paths from {idx_cach_path}")
            valid_cache_paths = pickle.load(open(idx_cach_path, "rb"))
        ...
        # Save cache index
        print(f"Saving valid cache paths to {idx_cach_path}")
        pickle.dump(valid_cache_paths, open(idx_cach_path, "wb"))
